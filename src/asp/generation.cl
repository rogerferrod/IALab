...
% TODO: relazione -> scrivere il perche degli 0 e degli 1
...
:- 2 != #count{D : calendar(W, D, _, _, _, _)}, week(W), not fullweek(W).
%test1(W, X) :- X = #count{D : calendar(W, D, _, _, _, _)}, week(W), not fullweek(W).
%#show test1/2.

% settimane full hanno 6 giorni
:- 6 != #count{D : calendar(W, D, _, _, _, _)}, fullweek(W).
%test2(W, X) :- X = #count{D : calendar(W, D, _, _, _, _)}, fullweek(W).
%#show test2/2.

% il sabato ha 4 o 5 ore
% TODO: da controllare la sitassi
:- #count{I : calendar(W, D, _, I, _, _)} > 5, week(W), day(D), not fullday(D).
:- #count{I : calendar(W, D, _, I, _, _)} < 4, week(W), day(D), not fullday(D).
%test3(W, X) :- X = #count{I : calendar(W, D, _, I, _, _)}, week(W), day(D), not fullday(D).
%#show test3/2.

% I giorni delle settimane fullweek (lun-ven) hanno 8 ore
:- 8 != #count{I : calendar(W, D, _, I, _, _)}, fullweek(W), fullday(D).
%test4(W, D, X) :- X = #count{I : calendar(W, D, _, I, _, _)}, week(W), day(D).
%#show test4/3.

% I venerdì delle settimane "normali" (non fullweek) hanno 8 ore
:- 8 != #count{I : calendar(W, 5, _, I, _, _)}, week(W), not fullweek(W).
%test5(W, D, X) :- X = #count{I : calendar(W, D, _, I, _, _)}, week(W), day(D).
%#show test5/3.
